...
def gen_samp_coords(grid_size=[10, 10], offv= 0.5):

    grid_h = np.arange(grid_size[1], dtype=np.float32)
    grid_w = np.arange(grid_size[0], dtype=np.float32)
    grid = np.meshgrid(grid_w, grid_h) # here w goes first
    grid = np.stack(grid, axis=0).reshape(2, grid_size[1]*grid_size[0]) + offv
    grid = torch.tensor(grid).permute(1, 0) / torch.tensor([[grid_size[0],  grid_size[1]]])
    return grid
...
def get_cls_embed(layer1, layer2, layer3, output, coords, sizes, wcl):
    ...
    b, num, _ = coords.shape
    coords = coords.view(-1, 1, num, 2) * 2 - 1
...
class REHEADV12(nn.Module):
    ...
    def __init__(self,  c1=128, c2=256, c3=512, outc=256):
        ...
        self.convck1 = A2C2f(c1=c3+c2, c2=c2, n=2 , a2=False, area=-1)
        self.unsample2 = nn.Upsample(scale_factor=2, mode ='nearest')

        self.convck2 = A2C2f(c1=c2+c1, c2=c2, n=2 , a2=False, area=-1)
        self.out1 = Conv(c1=c2, c2=outc//4, k=3, s=1)
        self.conv1 = Conv(c1=c2, c2=c2, k=3, s=2)


        self.convck3 = A2C2f(c1=c2+c2, c2=c2, n=2 , a2=False, area=-1)
        self.out2 = Conv(c1=c2, c2=outc//2, k=3, s=1)
        self.conv2 = Conv(c1=c2, c2=c2, k=3, s=2)

        self.convck4 =C3k2(c1=c3+c2, c2=c2, n=2 , c3k=True)
        self.out3 = Conv(c1=c2, c2=outc, k=3, s=1)
...
class REHEADV10(nn.Module):
    ...
    def __init__(self,  c1=128, c2=256, c3=512, outc=256):
        ...
        self.unsample1 = nn.Upsample(scale_factor=2, mode ='nearest')
        self.convck1 = C2fCIB(c1=c3+c2, c2=c2, n=3 , shortcut=True)
        self.unsample2 = nn.Upsample(scale_factor=2, mode ='nearest')

        self.convck2 = C2f(c1=c2+c1, c2=c2, n=3)
        self.out1 = Conv(c1=c2, c2=outc//4, k=3, s=1)
        self.conv1 = Conv(c1=c2, c2=c2, k=3, s=2)


        self.convck3 = C2fCIB(c1=c2+c2, c2=c2, n=3 , shortcut=True)
        self.out2 = Conv(c1=c2, c2=outc//2, k=3, s=1)
        self.conv2 = Conv(c1=c2, c2=c2, k=3, s=2)

        self.convck4 =C2fCIB(c1=c3+c2, c2=c2, n=3 , shortcut=True)
        self.out3 = Conv(c1=c2, c2=outc, k=3, s=1)
...
class REHEADV8(nn.Module):
    ...
    def __init__(self,  c1=128, c2=256, c3=512, outc=256):
        ...
        self.unsample1 = nn.Upsample(scale_factor=2, mode ='nearest')
        self.convck1 = C2f(c1=c3+c2, c2=c2, n=3)
        self.unsample2 = nn.Upsample(scale_factor=2, mode ='nearest')
        ...
        self.convck3 = C2f(c1=c2+c2, c2=c2, n=3)
        self.out2 = Conv(c1=c2, c2=outc//2, k=3, s=1)
        self.conv2 = Conv(c1=c2, c2=c2, k=3, s=2)

        self.convck4 =C2f(c1=c3+c2, c2=c2, n=3)
        self.out3 = Conv(c1=c2, c2=outc, k=3, s=1)
...
class REHEADV5(nn.Module):
    ...
    def __init__(self,  c1=128, c2=256, c3=512, outc=256):
        ...
        self.unsample1 = nn.Upsample(scale_factor=2, mode ='nearest')
        self.convck1 = C3(c1=c3+c2, c2=c2, n=3)

        self.conv2 = Conv(c1=c2, c2=c2, k=1, s=1)
        self.unsample2 = nn.Upsample(scale_factor=2, mode ='nearest')
        self.convck2 = C3(c1=c2+c1, c2=c2, n=3)

        self.out1 = Conv(c1=c2, c2=outc//4, k=3, s=1)
        self.conv3 = Conv(c1=c2, c2=c2, k=3, s=2)


        self.convck3 = C3(c1=c2+c2, c2=c2, n=3)
        self.out2 = Conv(c1=c2, c2=outc//2, k=3, s=1)
        self.conv4 = Conv(c1=c2, c2=c2, k=3, s=2)

        self.convck4 =C3(c1=c3+c2, c2=c2, n=3)
        self.out3 = Conv(c1=c2, c2=outc, k=3, s=1)
...
class REHEADV6(nn.Module):
    ...
    def __init__(self,  c1=128, c2=256, c3=512, outc=256):
        ...
        self.conv1 = Conv(c1=c3, c2=c2, k=1, s=1)
        self.unsample1 = nn.ConvTranspose2d(in_channels=c2,  out_channels=c2, kernel_size=2, stride=2)
        self.conv1_0 = Conv(c1=c2+c2, c2=c2, k=3, s=1)
        self.convck1 = nn.Sequential(*(Conv(c1=c2, c2=c2, k=3, s=1) for _ in range(9)))



        self.conv2 = Conv(c1=c2, c2=c2//2, k=1, s=1)
        self.unsample2 = nn.ConvTranspose2d(in_channels=c2//2,  out_channels=c2//2, kernel_size=2, stride=2)
        self.conv2_0 = Conv(c1=c2//2+c1, c2=c2//2, k=3, s=1)
        self.convck2 = nn.Sequential(*(Conv(c1=c2//2, c2=c2//2, k=3, s=1) for _ in range(9)))

        self.out1 = Conv(c1=c2//2, c2=outc//4, k=3, s=1)
        self.conv3 = Conv(c1=c2//2, c2=c2//2, k=3, s=2)
        self.conv3_0 = Conv(c1=c2+c2//2, c2=c2, k=3, s=1)
        self.convck3 = nn.Sequential(*(Conv(c1=c2, c2=c2, k=3, s=1) for _ in range(9)))


        self.out2 = Conv(c1=c2, c2=outc//2, k=3, s=1)

        self.conv4 = Conv(c1=c2, c2=c2, k=3, s=2)
        self.conv4_0 = Conv(c1=c3+c2, c2=c2*2, k=3, s=1)
        self.convck4 =nn.Sequential(*(Conv(c1=c2*2, c2=c2*2, k=3, s=1) for _ in range(9)))
        self.out3 = Conv(c1=c2*2, c2=outc, k=3, s=1)
...
class get_model(nn.Module):
    ...
    def __init__(self, num_layers, heads, head_conv=256):
        ...
        scalek  = {18: 1, 34: 1, 50: 4, 101: 4, 152: 4}
        oudim  = {18: 512, 34: 512, 50: 2048, 101: 2048, 152: 2048}
        self.scalek = scalek[num_layers]
        self.numchannel = 512
        self.nchannel = 512#self.numchannel

        model = FResNet(num_layers, heads, head_conv=head_conv)
        model.init_weights(num_layers)
        self.backone = model


        self.aifi = AIFI(c1=self.numchannel*self.scalek, cm=2048, num_heads=8)
        self.head = REHEADV11(c1=128*self.scalek, c2=256*self.scalek, c3=512*self.scalek, outc=self.numchannel*self.scalek)

        self.out1 = nn.Conv2d(in_channels= self.numchannel*self.scalek, out_channels=68, kernel_size=1, stride=1)
        ...
        self.sampv = torch.tensor([[0.05, 0.05], [-0.05, -0.05], [-0.05, 0.05], [0.05, -0.05]]
                                  ).reshape(1, 1, 4, 2).repeat(1, cfg.PointNms, 1, 1).cuda().float()
        self.sampcoords = gen_samp_coords(grid_size=[6, 6], offv= 0.5).unsqueeze(dim=0).cuda().float()

        ##################
        self.convm2 = nn.Conv2d(in_channels=128 * self.scalek, out_channels=64, kernel_size=1, stride=1,
                                bias=False)
        self.convm3 = nn.Conv2d(in_channels=256 * self.scalek, out_channels=128, kernel_size=1, stride=1,
                                bias=False)
        self.convm4 = nn.Conv2d(in_channels=512 * self.scalek, out_channels=256, kernel_size=1, stride=1,
                                bias=False)
        #####################

        self.silu = nn.SiLU()
        self.convF1 = nn.Conv2d(in_channels=64*self.scalek, out_channels=self.nchannel, kernel_size=1, stride=1, bias=False)
        ...
        self.bnF1 = nn.BatchNorm2d(self.nchannel, momentum=BN_MOMENTUM)
        ...
        self.convP1 = nn.Conv2d(in_channels=64*self.scalek, out_channels=self.nchannel, kernel_size=1, stride=1, bias=False)
        self.convP2 = nn.Conv2d(in_channels=128*self.scalek, out_channels=self.nchannel, kernel_size=1, stride=1, bias=False)
        self.convP3 = nn.Conv2d(in_channels=256*self.scalek, out_channels=self.nchannel, kernel_size=1, stride=1, bias=False)
        self.convP4 = nn.Conv2d(in_channels=512*self.scalek, out_channels=self.nchannel, kernel_size=1, stride=1, bias=False)
        ###################
        ...
        self.pos1 = nn.Parameter(torch.zeros(1, self.Tsize[0] * 8, self.Tsize[1] * 8,  self.numchannel // 8*self.scalek), requires_grad=False)
        ...
        self.InitEncoder = nn.ModuleList([
            ...
            for i in range(4)])
        # self.InitEncoder = nn.ModuleList([
        #     Transformer(dim=self.nchannel, num_heads=self.nchannel // 64, mlp_ratio=2, qkv_bias=True, norm_layer=norm_layer)
        #     for i in range(4)])
    ...
    def initialize_weights(self):
        ...
        pos1 = pos1.reshape(self.Tsize[0] * 8, self.Tsize[1] * 8,  self.numchannel//8*self.scalek)
    ...
    def forward(self, x):
        ...
        # layer3 = self.convm3(layer3)
        # layer4 = self.convm4(layer4)
        ...
        ############sample############
        layer4_ = F.interpolate(layer4, (self.TsizeS[0], self.TsizeS[1]), mode= "bilinear").view(b, c, h * w).permute(0, 2, 1)
        pos4_ = F.interpolate(pos4, (self.TsizeS[0], self.TsizeS[1]), mode= "bilinear").view(b, c, h * w).permute(0, 2, 1)
        ############sample############
        ...
        #################################################################
        # sampv =  self.sampcoords.repeat(b, 1, 1)
        # initcoords = torch.concatenate([sampv, output1[:, :, :2].detach()], dim=1)
        ...
        #
        # # inint_c = get_cls_embed(layer1, layer2, layer3, layer4, initcoords, sizes, self.wcl1)
        # # inint_p = get_cls_embed(pos1, pos2, pos3, pos4, initcoords, sizes, self.wcl2)
        #
        # #pclsp_embed = inint_p.clone()
        # #pclsp_embed[:, -cfg.PointNms:,:] = self.pcls_embed.expand(b, -1, -1) + inint_p[:, -cfg.PointNms:,:]* self.inintwp[0]
        ...
        #
        #     # pclsp_embed = inint_p.clone()
        #     # pclsp_embed[:, -cfg.PointNms:,:] = self.pcls_embed.expand(b, -1, -1) + inint_p_* self.inintwp[i+1]#
        #     # xc_[:, -cfg.PointNms:,:]= xc_[:, -cfg.PointNms:,:]+ inint_c_ * self.inintwc[i+1]#
        #
