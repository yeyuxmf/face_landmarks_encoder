...
from torch_geometric.nn import GATConv, ASAPooling, GATv2Conv, PairNorm
BN_MOMENTUM = 0.1
...
edgeindex = []
for i in range(cfg.PointNms):
    for j in range(cfg.PointNms):
        edgeindex.append([i, j])
edgeindex = np.array(edgeindex).swapaxes(0, 1)
batch = np.array([0 for bi in range(cfg.PointNms)])

def get_edge_index(batch_size):
    edgeindexs = []
    batch_ = []
    for b in range(batch_size):
        edgeindexs.append(edgeindex + (cfg.PointNms) * b)
        batch_.append(batch + b)
    edgeindexs = torch.tensor(np.concatenate(edgeindexs, axis=-1)).cuda().long()
    batch_ = torch.tensor(np.array(batch_).reshape(batch_size * cfg.PointNms)).cuda().long()

    return edgeindexs, batch_
...
class GATCONV(nn.Module):
    def __init__(self, inchannel, onchannel, dropout=0.0):
        super(GATCONV, self).__init__()
        self.conv = GATv2Conv(inchannel, onchannel, heads=1, concat=False, dropout=0.0)
        self.bn = PairNorm()
...
class get_model(nn.Module):
    ...
    def __init__(self, num_layers, heads, head_conv=256):
        ...
        self.numchannel = 512
        nchannel = self.numchannel

        model = FResNet(num_layers, heads, head_conv=head_conv)
        model.init_weights(num_layers)
        self.backone = model

        self.out1 = nn.Conv2d(in_channels= self.numchannel, out_channels=1, kernel_size=1, stride=1)

        self.Tsize = torch.tensor([8, 8]).int()
        self.TsizeS = self.Tsize*2
        self.pcls_embed = nn.Parameter(torch.zeros(1, cfg.PointNms, nchannel), requires_grad=False)

        self.convF1 = nn.Conv2d(in_channels=64, out_channels=nchannel, kernel_size=1, stride=1, bias=False)
        self.convF2 = nn.Conv2d(in_channels=128, out_channels=nchannel, kernel_size=1, stride=1, bias=False)
        self.convF3 = nn.Conv2d(in_channels=256, out_channels=nchannel, kernel_size=1, stride=1, bias=False)
        self.convF4 = nn.Conv2d(in_channels=512, out_channels=nchannel, kernel_size=1, stride=1, bias=False)
        self.bnF1 = nn.BatchNorm2d(nchannel, momentum=BN_MOMENTUM)
        self.bnF2 = nn.BatchNorm2d(nchannel, momentum=BN_MOMENTUM)
        self.bnF3 = nn.BatchNorm2d(nchannel, momentum=BN_MOMENTUM)
        self.bnF4 = nn.BatchNorm2d(nchannel, momentum=BN_MOMENTUM)
        self.convP1 = nn.Conv2d(in_channels=64, out_channels=nchannel, kernel_size=1, stride=1, bias=False)
        self.convP2 = nn.Conv2d(in_channels=128, out_channels=nchannel, kernel_size=1, stride=1, bias=False)
        self.convP3 = nn.Conv2d(in_channels=256, out_channels=nchannel, kernel_size=1, stride=1, bias=False)
        self.convP4 = nn.Conv2d(in_channels=512, out_channels=nchannel, kernel_size=1, stride=1, bias=False)
        ...
        self.pos1 = nn.Parameter(torch.zeros(1, self.Tsize[0] * 8, self.Tsize[1] * 8,  self.numchannel // 8), requires_grad=False)
        self.pos2 = nn.Parameter(torch.zeros(1, self.Tsize[0] * 4, self.Tsize[1] * 4,  self.numchannel // 4),requires_grad=False)
        self.pos3 = nn.Parameter(torch.zeros(1, self.Tsize[0] * 2, self.Tsize[1] * 2,  self.numchannel // 2),requires_grad=False)
        self.pos4 = nn.Parameter(torch.zeros(1, self.Tsize[0],  self.Tsize[1],  self.numchannel),requires_grad=False)  # fixed sin-cos embedding
        norm_layer = partial(nn.LayerNorm, eps=1e-6)

        self.InitEncoder = nn.ModuleList([
            Block(dim=nchannel, num_heads=nchannel // 64, mlp_ratio=2, qkv_bias=True, norm_layer=norm_layer)
            for i in range(4)])
        self.linear11 = nn.Linear(nchannel, 256)
        self.linear12 = nn.Linear(256, 4)

        # self.DEncoder = nn.ModuleList([
        #     Block(dim=nchannel, num_heads=nchannel // 64, mlp_ratio=2, qkv_bias=True, norm_layer=norm_layer)
        #     for i in range(4)])
        self.DEncoder = nn.ModuleList([GATCONV(inchannel=nchannel, onchannel=nchannel, dropout=0.0)
            for i in range(4)])
        ...
        fc_coord_branch = []
        for _ in range(2):
            fc_coord_branch.append(nn.Linear(nchannel, nchannel))
            fc_coord_branch.append(nn.ReLU())
        fc_coord_branch.append(nn.Linear(nchannel, 2))
        ...
        fc_sigma_branch = []
        for _ in range(2):
            fc_sigma_branch.append(nn.Linear(nchannel, nchannel))
        fc_sigma_branch.append(Linear_with_norm(nchannel, 2, norm=False))
    ...
    def initialize_weights(self):
        ...
        pos1 = pos1.reshape(self.Tsize[0] * 8, self.Tsize[1] * 8,  self.numchannel//8)
        ...
        pos2 = pos2.reshape(self.Tsize[0] * 4, self.Tsize[1] * 4, self.numchannel//4)
        ...
        pos3 = pos3.reshape(self.Tsize[0] * 2, self.Tsize[1] * 2, self.numchannel//2)
        ...
        pos4 = pos4.reshape(self.Tsize[0], self.Tsize[1], self.numchannel)
    ...
    def forward(self, x):
        ...
        sizes = [b, c, h, w]
        #################################################################
        inc = torch.cat([layer4_, torch.zeros((b, cfg.PointNms, self.numchannel)).cuda().float()], dim=1)
        ...
        edgeindexs, batch_ = get_edge_index(b)
        for i, cpb in enumerate(self.DEncoder):
            xc_ = cpb(xc_.reshape(b*cfg.PointNms, -1), edgeindexs, batch_)
            xc_ = xc_.reshape(b, cfg.PointNms, -1)
