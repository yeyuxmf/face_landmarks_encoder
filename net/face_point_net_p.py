...
def get_cls_embed(layer1, layer2, layer3, output, coords, sizes, wcl):
    ...
    cls1 = F.grid_sample(layer1, coords, align_corners=True)
    cls2 = F.grid_sample(layer2, coords, align_corners=True)
    cls3 = F.grid_sample(layer3, coords, align_corners=True)
    cls4 = F.grid_sample(output, coords, align_corners=True)
...
class get_model(nn.Module):
    ...
    def __init__(self, num_layers, heads, head_conv=256):
        ...
        self.out1 = nn.Conv2d(in_channels=self.numchannel // 4, out_channels=1, kernel_size=1, stride=1)
        ...
        self.pos1 = nn.Parameter(torch.zeros(1, self.Tsize[0] * 8, self.Tsize[1] * 8, self.numchannel // 8),
                                 requires_grad=False)
        self.pos2 = nn.Parameter(torch.zeros(1, self.Tsize[0] * 4, self.Tsize[1] * 4, self.numchannel // 4),
                                 requires_grad=False)
        self.pos3 = nn.Parameter(torch.zeros(1, self.Tsize[0] * 2, self.Tsize[1] * 2, self.numchannel // 2),
                                 requires_grad=False)
        self.pos4 = nn.Parameter(torch.zeros(1, self.Tsize[0], self.Tsize[1], self.numchannel),
