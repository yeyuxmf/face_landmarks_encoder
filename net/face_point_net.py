...
class get_model(nn.Module):
    ...
    def __init__(self, num_layers, heads, head_conv=256):
        ...
        self.Tsize = torch.tensor([7, 7]).int()
        ...
        self.inintwc = nn.Parameter(torch.tensor([1.0]), requires_grad=True)
        self.inintwp = nn.Parameter(torch.tensor([1.0]), requires_grad=True)
    ...
    def forward(self, x):
        ...
        #################################################################
        xc = layer4_ * self.wg[0] + layer1_ * self.wg[1] + layer2_ * self.wg[2] + layer3_ * self.wg[3]
        pos = pos4_ * self.pg[0] + pos1_ * self.pg[1] + pos2_ * self.pg[2] + pos3_ * self.pg[3]
        ...
        pclsc_embed = inint_c
        pclsp_embed = self.pcls_embed.expand(b, -1, -1) + inint_p * self.inintwp


        hs = []
        inter_references = []
        xc_ = torch.cat([xc, pclsc_embed], dim=1)
        pos_ = torch.cat([pos, pclsp_embed], dim=1)
        xc_ = xc_ + pos_
        ...
        for i, cpb in enumerate(self.DEncoder):
            ...
            pclsp_embed = self.pcls_embed.expand(b, -1, -1) + inint_p * self.inintwp
            pos_ = torch.cat([pos, pclsp_embed], dim=1)
            xc_[:, -cfg.PointNms:, :] = xc_[:, -cfg.PointNms:, :] + pclsc_embed* self.inintwc
            xc_ = xc_ + pos_
