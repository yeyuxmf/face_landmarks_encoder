...
class GetRandomGeometryMatrix:
    ...
    def process(self):
        ...
        if self.shift_config[:2] != (0.0, 0.0) or \
            ...
            shift_xy = self._random(self.shift_config, size=[2]) * min(to_h, to_w)
...
class RandomTexture:
    def __init__(self, random_gray_rate=0, random_occ_rate=0, random_blur_rate=0, random_gamma_rate=0,
                 random_nose_fusion_rate=0):
...
def crop_face(image, landmarks, sacle=None):
    # 获取裁剪参数
    ...
    # 对图像进行裁剪
    imageC = image[y1: y2, x1: x2, :].copy()

    # 获取裁剪后的图像形状
    img_shape = imageC.shape
    # 对关键点坐标进行裁剪后的调整
    landmarksC = landmarks - np.array([[x1, y1]])
    # 归一化关键点坐标
    landmarksC = landmarksC / np.array([img_shape[1], img_shape[0]])
    return imageC, landmarksC
