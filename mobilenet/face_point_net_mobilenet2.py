...
from mobilenet.mobilenetv3 import MobileNetV3_Large
...
class get_model(nn.Module):
    ...
    def __init__(self):
        ...
        self.scalek = 1
        self.numchannel = 128
        self.nchannel = 384#self.numchannel

        model = MobileNetV3_Large()
        model.load_state_dict(torch.load("./mobilenet/450_act3_mobilenetv3_large.pth", map_location='cpu'))
        self.backone = model

        self.aifi = AIFI(c1=160, cm=640, num_heads=8)
        self.head = REHEADV11(c1=40*self.scalek, c2=112*self.scalek, c3=160*self.scalek, outc=self.nchannel*self.scalek)

        self.out1 = nn.Conv2d(in_channels= 160*self.scalek, out_channels=68, kernel_size=1, stride=1)
